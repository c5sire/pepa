...
```{r , echo = FALSE, results='hide'}
 ...
 if (stringr::str_detect(meta$title, "PT")) {phs_lbl = "Preliminary Trial"}
 if (stringr::str_detect(meta$title, "OT")) {phs_lbl = "Observation Trial"}
 brp_lbl = "Yield Breeding Program"
```

This sweetpotato trial has the identifier `r meta$title`. It was conducted under the supervion of `r meta$contact`  as a `r phs_lbl` as part of a `r brp_lbl` in `r meta$site`, `r meta$country` in `r meta$year`. A total of `r length(meta$materials)` clones (including reference clones) were evaluated for `r meta$variables` traits.
